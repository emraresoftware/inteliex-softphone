...
class DeviceSoftphonePersistence implements SoftphonePersistence {
  DeviceSoftphonePersistence({FlutterSecureStorage? secureStorage})
      : _secureStorage = secureStorage ?? _makeSecureStorage();
  ...
  /// Android Keystore bazı güncelleme senaryolarında (MIUI, One UI) anahtarı
  /// geçersiz kılabilir. resetOnError=true ile depolama sesi çökmüyor,
  /// boşaltan storage yerine _passwordMirror (SharedPreferences) devreye giriyor.
  static const _androidOptions = AndroidOptions(
    encryptedSharedPreferences: true,
    resetOnError: true,
  );

  static FlutterSecureStorage _makeSecureStorage() =>
      const FlutterSecureStorage(aOptions: _androidOptions);

  // Mirror degerleri SharedPreferences'ta durdugu icin duz metin birakmiyoruz.
  // Bu guclu kripto degil, karartmadir (obfuscation): yedek/aktarim dosyalarini
  // kabaca tarayan araclara karsi koruma saglar. Gercek sir Keystore'dadir;
  // mirror yalnizca Keystore sifirlanirsa devreye giren son care kopyadir.
  static const String _mirrorEncodedPrefix = 'enc1:';
  static final List<int> _mirrorKeyBytes =
      utf8.encode('inteliex.softphone.mirror.v1');

  static String _encodeMirror(String value) {
    final bytes = utf8.encode(value);
    final mixed = List<int>.generate(
      bytes.length,
      (i) => bytes[i] ^ _mirrorKeyBytes[i % _mirrorKeyBytes.length],
    );
    return '$_mirrorEncodedPrefix${base64Encode(mixed)}';
  }

  static String? _decodeMirror(String? raw) {
    if (raw == null || raw.isEmpty) {
      return null;
    }
    if (!raw.startsWith(_mirrorEncodedPrefix)) {
      // Eski surumlerin duz metin yazdigi mirror kayitlari.
      return raw;
    }
    try {
      final data = base64Decode(raw.substring(_mirrorEncodedPrefix.length));
      final mixed = List<int>.generate(
        data.length,
        (i) => data[i] ^ _mirrorKeyBytes[i % _mirrorKeyBytes.length],
      );
      return utf8.decode(mixed);
    } catch (error) {
      debugPrint('Softphone mirror decode failed: $error');
      return null;
    }
  }
  ...
  Future<void> saveAccounts({
    ...
    return _enqueueWrite(() async {
      ...
      for (final account in accounts) {
        ...
        await prefs.setString(mirrorKey, _encodeMirror(account.password));
      }
  ...
  Future<_DecodedAccounts> _decodeAccounts(String? raw) async {
    ...
    for (final item in decoded) {
      ...
      if (passwordMissing) {
        final mirrorPassword =
            _decodeMirror(prefs.getString(_passwordMirrorKey(id)));
  ...
  SipAccount? _normalizeStoredAccount(
    ...
    final normalizedAuthorizationUser = account.authorizationUser.trim().isNotEmpty
        ? account.authorizationUser.trim()
        : normalizedUsername;
    final normalizedDomain = AppEnvironment.normalizeHost(
    ...
    return account.copyWith(
      ...
      stunServer: account.stunServer.trim(),
      // Optimistik UI: önceki oturumda kayıtlıysa "registered" varsay; yeniden
      // register sırasında onRegistrationUpdate debounce'u "connecting"i geçici
      // olarak görmezden gelir, kullanıcı flicker yaşamaz. Şifre eksikse Hatalı.
