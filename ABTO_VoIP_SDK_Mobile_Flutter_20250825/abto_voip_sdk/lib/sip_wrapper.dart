...
class SipWrapper {
  ...
  static const String _EVENT_SET_SENDING_RTP_AUDIO = "setSendingRtpAudio";
  ...
  int? _lastRegistrationFailureCode;
  String? _lastRegistrationFailureText;

  bool get isRegistered => _isRegistered;
  int? get lastRegistrationFailureCode => _lastRegistrationFailureCode;
  String? get lastRegistrationFailureText => _lastRegistrationFailureText;
  ...
  Future<String> getVersion() {
    ...
    return _channel?.platform
            ?.invokeMethod<String>(_EVENT_LIB_VERSION)
            .then<String>((String? value) => value ?? "error null") ??
        Future.value("error empty");
  }

  Future<AbtoPhoneCfg> getConfigs() {
    debugPrint("SipWrapper getConfigs");
    Future<AbtoPhoneCfg>? result = _channel?.platform
        ?.invokeMethod<Map<Object?, Object?>>(_EVENT_GET_CONFIGS)
        .then((value) {
      AbtoPhoneCfg configs = AbtoPhoneCfg();
      if (value?.isEmpty ?? true) {
        debugPrint(
            "ERROR: config is null, verify license or whether it is set before other methods are called");
        return configs;
      }
      ...
      configs.audioCodecs =
          (value?["audioCodecs"] as String).parseAudioCodecs();
      configs.videoCodecs =
          (value?["videoCodecs"] as String).parseVideoCodecs();
      configs.isEnabledAutoSendRtpVideo =
          value?["isEnabledAutoSendRtpVideo"] as bool;
      return configs;
    });
  ...
  void register(String domain, String proxy, String user, String pass,
      String authId, String displName, int expire) {
    // Each explicit register attempt should emit a fresh registration outcome.
    _isRegistered = false;
  ...
  void startCall(String number, bool isVideo) {
    debugPrint(
        "SipWrapper startCall to [" + number + "] video " + isVideo.toString());
  ...
  void setLogLevel(int logLevel, bool toFile) {
    ...
    _channel?.platform
        ?.invokeMethod(_EVENT_SET_LOG_LEVEL, [logLevel, toFile ? 1 : 0]);
  }
  ...
  void setSendingRtpAudio(bool enable) {
    debugPrint("SipWrapper setSendingRtpAudio: $enable");
    _channel?.platform?.invokeMethod(_EVENT_SET_SENDING_RTP_AUDIO, enable);
  }
  ...
  void _stateChanged() {
    ...
    switch (_channel?.methodName) {
      case _RESULT_REGISTERED:
        ...
        _isRegistered = true;
        _lastRegistrationFailureCode = null;
        _lastRegistrationFailureText = null;
      ...
      case _RESULT_REG_FAILED:
        if (_isRegistered) return;
        final args = _channel?.arguments;
        if (args is Map) {
          final code = args['statusCode'];
          final text = args['statusText'];
          if (code is int) {
            _lastRegistrationFailureCode = code;
          } else if (code is num) {
            _lastRegistrationFailureCode = code.toInt();
          } else if (code is String) {
            _lastRegistrationFailureCode = int.tryParse(code.trim());
          }
          if (text is String && text.trim().isNotEmpty) {
            _lastRegistrationFailureText = text.trim();
          }
        }
      ...
      case _RESULT_MSG_RECEIVED:
        ...
        textMessageListener?.onTextMessageReceived
            ?.call(fromNumber, toNumber, message);
        break;
      ...
      case _RESULT_MSG_STATUS:
        ...
        textMessageListener?.onTextMessageStatus
            ?.call(address, reason, success);
        break;
...
class TextMessageListener {
  ...
  void Function(String address, String reason, bool success)?
      onTextMessageStatus;
}
